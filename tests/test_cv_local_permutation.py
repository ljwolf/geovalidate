...
from geovalidate.cv import LocalPermutation


@pytest.fixture
def grid_gdf():
    """4 * 4 regular grid with unit spacing -- each site has 2-4 neighbors
    within threshold=1.5."""
...
def test_is_valid_permutation(grid_gdf):
    ...
    n = len(grid_gdf)
    lp = LocalPermutation(threshold=1.5, derangement=False,
...
def test_distance_constraint_derangement(grid_gdf, coords):
    lp = LocalPermutation(threshold=1.5, derangement=True,
                           n_permutations=20, random_state=0)
    for perm in lp.sample(grid_gdf):
        for i, j in enumerate(perm):
            d = numpy.linalg.norm(coords[i] - coords[j])
...
def test_infeasible_derangement_raises():
    ...
    lp = LocalPermutation(threshold=1.0, derangement=True, n_permutations=1)
...
def test_infeasible_permutation_raises():
    ...
    # threshold=1 -- no pair within range, self-assignment also blocked
    lp = LocalPermutation(threshold=0.5, derangement=True, n_permutations=1)
    with pytest.raises(ValueError, match="No valid"):
        list(lp.sample(gdf))
...
def test_reproducible(grid_gdf):
    lp1 = LocalPermutation(threshold=1.5, derangement=True,
                            n_permutations=10, random_state=99)
    lp2 = LocalPermutation(threshold=1.5, derangement=True,
                            n_permutations=10, random_state=99)
...
def test_1d_time_series_input():
    ...
    lp = LocalPermutation(threshold=2.5, derangement=True,
                           n_permutations=5, random_state=0)
...
def test_pandas_series_input():
    ...
    lp = LocalPermutation(threshold=2.5, n_permutations=3, random_state=0)
    for perm in lp.sample(t):
...
def test_pandas_series_datetime_index():
    ...
    lp = LocalPermutation(threshold=3 * 86400, n_permutations=3, random_state=0)
    for perm in lp.sample(t):
...
def test_geometry_array_input():
    ...
    lp = LocalPermutation(threshold=3.0, n_permutations=3, random_state=0)
...
def test_no_threshold_no_graph_raises():
...
def test_get_params():
    p = LocalPermutation(threshold=500.0, derangement=False,
                          n_permutations=99).get_params()
    assert p["threshold"] == 500.0
