"""Shared helpers for the cv subpackage."""

import numpy
import geopandas


# Maps our kernel names to the equivalent libpysal kernel name.
# All kernels we expose have a libpysal equivalent.
LIBPYSAL_KERNEL_MAP: dict[str, str] = {
    "gaussian":    "gaussian",
    "exponential": "exponential",
    "bisquare":    "bisquare",
    "triangular":  "triangular",
    "uniform":     "boxcar",
    "parabolic":   "parabolic",
}


def _to_point_gdf(X) -> geopandas.GeoDataFrame:
    """Coerce any geo input to a Point GeoDataFrame for libpysal graph builders.

    Accepts GeoDataFrame, GeoSeries, or GeometryArray.
    Polygon/multipolygon geometries are replaced by their centroids.
    """
    from geopandas.array import GeometryArray

    if isinstance(X, GeometryArray):
        gdf = geopandas.GeoDataFrame(geometry=geopandas.GeoSeries(X))
    elif isinstance(X, geopandas.GeoSeries):
        gdf = geopandas.GeoDataFrame(geometry=X)
    else:
        gdf = X.copy()
    if not (gdf.geom_type == "Point").all():
        gdf = gdf.copy()
        gdf["geometry"] = gdf.geometry.centroid
    return gdf


def _idx_and_is_geo(X):
    """Return (index_labels_or_None, is_spatial).

    Index labels are returned for pandas Series, GeoSeries, and GeoDataFrame
    so callers can map positional results back to the original index.
    is_spatial controls whether a libpysal spatial graph or a 1-D time-series
    distance is used to build the weight matrix.
    """
    import pandas
    from geopandas.array import GeometryArray

    if isinstance(X, (geopandas.GeoDataFrame, geopandas.GeoSeries)):
        return X.index.to_numpy(), True
    if isinstance(X, GeometryArray):
        return None, True
    if isinstance(X, pandas.Series):
        return X.index.to_numpy(), False
    return None, False


def _get_coords(X) -> numpy.ndarray:
    """Return an (n, d) float array of coordinates from any supported input.

    GeoDataFrame / GeoSeries / GeometryArray -> (n, 2) centroid x, y
    pandas.Series  -> (n, 1) using index values as time positions;
                     DatetimeIndex is converted to seconds from first entry
    1-D ndarray    -> (n, 1) time-series (distance = scalar lag)
    (n, 1)/(n, 2) ndarray -> passed through as float64
    """
    import pandas
    from geopandas.array import GeometryArray

    if isinstance(X, geopandas.GeoDataFrame):
        c = X.geometry.centroid
        return numpy.column_stack([c.x, c.y])
    if isinstance(X, geopandas.GeoSeries):
        c = X.centroid
        return numpy.column_stack([c.x, c.y])
    if isinstance(X, GeometryArray):
        c = geopandas.GeoSeries(X).centroid
        return numpy.column_stack([c.x, c.y])
    if isinstance(X, pandas.Series):
        idx = X.index
        if isinstance(idx, pandas.DatetimeIndex):
            t = (idx - idx[0]).total_seconds().to_numpy()
        else:
            t = numpy.asarray(idx, dtype=float)
        return t.reshape(-1, 1)
    arr = numpy.asarray(X, dtype=float)
    if arr.ndim == 1:
        return arr.reshape(-1, 1)
    if arr.ndim == 2 and arr.shape[1] in (1, 2):
        return arr
    raise ValueError(
        "Array-like input must be shape (n,), (n, 1), or (n, 2). "
        "Pass a 1-D array for time-series data (distance = absolute lag)."
    )


# Kernel functions K(t) where t = d / bandwidth.
# All return 0 for t >= 1 except Gaussian and Exponential (infinite support).
KERNELS: dict = {
    "gaussian":    lambda t: numpy.exp(-0.5 * t ** 2),
    "exponential": lambda t: numpy.exp(-t),
    "bisquare":    lambda t: numpy.where(t < 1.0, (1.0 - t ** 2) ** 2, 0.0),
    "triangular":  lambda t: numpy.where(t < 1.0, 1.0 - t, 0.0),
    "uniform":     lambda t: numpy.where(t < 1.0, 1.0, 0.0),
    "parabolic":   lambda t: numpy.where(t < 1.0, 0.75 * (1.0 - t ** 2), 0.0),
}
